...
-- 日本私立中学校 包括的基盤データベース
-- 作成日: 2026-05-05
-- 前提: SQLite 3.x + FTS5 + JSON1
-- 既存DB（mg, it等）と同型のID命名規則: jpms_<entity>_<NNNN>
-- 全テーブルは source_id 経由で jpms_sources を参照（一次情報トレース）
...
CREATE TABLE jpms_sources (
    ...
    source_type TEXT CHECK(
        source_type IN ('school_website','school_brochure','school_book',
                        'interview_field','newspaper','academic_paper','academic_book',
                        'official_stats','sns_x','sns_instagram','sns_youtube',
                        'blog','5ch','other')
    ) NOT NULL,
    title TEXT,
    author TEXT,
    ...
    rights_status TEXT CHECK(
        rights_status IN ('public_domain','cc_by','cc_by_nc',
                          'copyrighted_quotable','copyrighted_permission_required',
                          'private_use_only','unknown')
    ),
...
CREATE TABLE jpms_school_outcomes (
    ...
    outcome_type TEXT CHECK(
        outcome_type IN ('todai_kyodai','soukei','gmarch','kankan_doritsu',
                         'medical','overseas','attached_uni','other')
    ),
    count INTEGER,
    note TEXT,
    source_id TEXT REFERENCES jpms_sources(id)
...
CREATE TABLE jpms_testimonials (
    ...
    speaker_category TEXT CHECK(
        speaker_category IN ('student_current','student_former','parent_current','parent_former',
                             'teacher','principal','external_evaluator','third_party')
    ) NOT NULL,
    speaker_name TEXT,
    speaker_anonymized INTEGER DEFAULT 1,
    spoken_year INTEGER,
    medium TEXT CHECK(
        medium IN ('school_website','school_brochure','school_event','interview',
                   'book','newspaper','x','instagram','youtube','blog','note','5ch','other')
    ) NOT NULL,
    ...
    rights_level TEXT CHECK(
        rights_level IN ('public','quoted_with_attribution','anonymized_only',
                         'permission_required','withhold')
    ) NOT NULL,
...
CREATE TABLE jpms_education_concepts (
    ...
    subfield TEXT NOT NULL CHECK(
        subfield IN ('learning_science','motivation_dev','noncognitive','sel',
                     'adolescent_dev','career_dev','wellbeing','evidence_based',
                     'comparative','japanese_pedagogy','curriculum','assessment',
                     'inclusion','sociology_of_ed')
    ),
...
CREATE TABLE jpms_education_concept_relations (
    ...
    relation_type TEXT NOT NULL CHECK(
        relation_type IN ('derived_from','extends','critiques','empirically_tests',
                          'synthesizes','competes_with','applies_to_practice',
